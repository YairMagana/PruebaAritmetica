...
namespace PruebaAritmetica.Clases
{
    internal class TestCR
    {
        ...
        public void Test(int prec)
        {
            ...
            _consoleManager.WriteLine($"{x} = {cr1.toStringFloatRep(4, 10, 1).toString()}");
            _consoleManager.WriteLine($"{y} = {cr2.toStringFloatRep(1, 10, 1).toString()}");
            ...
            string val2 = "0.000000000987654321";
            string val3 = "-123456789.987654321";
        ...
        public static string ConvertToScientificNotation(string numberStr, int leftDigits, int rightDigits)
        {
            if (string.IsNullOrWhiteSpace(numberStr))
                throw new ArgumentException("Input string cannot be null or empty.", nameof(numberStr));
            ...
            {
                // Find the first non-zero digit in fractional part
                int firstNonZero = fractionalPart.IndexOfAny("123456789".ToCharArray());
                if (firstNonZero == -1)
                    return "0";

                // Exponent is negative and equal to -(position of first non-zero digit +1)
                int exponent = -(firstNonZero + 1);
                string significant = fractionalPart.Substring(firstNonZero);

                // Adjust significant digits based on leftDigits and rightDigits
                if (significant.Length < leftDigits)
                    leftDigits = significant.Length;

                string mantissaLeft = significant.Substring(0, leftDigits);
                string mantissaRight = significant.Length > leftDigits
                    ? significant.Substring(leftDigits, Math.Min(rightDigits, significant.Length - leftDigits))
                    : string.Empty;

                string mantissa = mantissaLeft;
                if (mantissaRight.Length > 0)
                    mantissa += "." + mantissaRight;
            ...
            else
            {
                ...
                string significant = integerPart + fractionalPart;
